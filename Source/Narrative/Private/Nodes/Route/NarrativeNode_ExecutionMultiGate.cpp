...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NarrativeNode_ExecutionMultiGate)

UNarrativeNode_ExecutionMultiGate::UNarrativeNode_ExecutionMultiGate(const FObjectInitializer& ObjectInitializer)
	...
	, StartIndex(INDEX_NONE)
{
	...
	InputPins.Add(FNarrativePin(TEXT("Reset"), ResetPinTooltip));
	SetNumberedOutputPins(0, 1);
	AllowedSignalModes = {ENarrativeSignalMode::Enabled, ENarrativeSignalMode::Disabled};
}

void UNarrativeNode_ExecutionMultiGate::ExecuteInput(const FName& PinName)
{
...
void UNarrativeNode_ExecutionMultiGate::Cleanup()
{
	NextOutput = 0;
	Completed.Reset();
}
...
#if WITH_EDITOR
FString UNarrativeNode_ExecutionMultiGate::GetNodeDescription() const
{
	FString Result;
	Result.Reserve(128);
