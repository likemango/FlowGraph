...
#define LOCTEXT_NAMESPACE "SNarrativeDiffControl"

/////////////////////////////////////////////////////////////////////////////
/// FNarrativeAssetDiffControl

FNarrativeAssetDiffControl::FNarrativeAssetDiffControl(const UNarrativeAsset* InOldNarrativeAsset, const UNarrativeAsset* InNewNarrativeAsset, FOnDiffEntryFocused InSelectionCallback)
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 2
	: TDetailsDiffControl(InOldNarrativeAsset, InNewNarrativeAsset, InSelectionCallback)
#else
	: FDetailsDiffControl(InOldNarrativeAsset, InNewNarrativeAsset, InSelectionCallback, false)
#endif
{
}

// TDetailsDiffControl::GenerateTreeEntries + "NoDifferences" entry + category label
void FNarrativeAssetDiffControl::GenerateTreeEntries(TArray<TSharedPtr<FBlueprintDifferenceTreeEntry>>& OutTreeEntries, TArray<TSharedPtr<FBlueprintDifferenceTreeEntry>>& OutRealDifferences)
{
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 2
	...
	static const FText AssetPropertiesTooltip = LOCTEXT("AssetPropertiesTooltip", "The list of changes made to Narrative Asset properties.");
...
/////////////////////////////////////////////////////////////////////////////
/// FNarrativeGraphToDiff

FNarrativeGraphToDiff::FNarrativeGraphToDiff(SNarrativeDiff* InDiffWidget, UEdGraph* InGraphOld, UEdGraph* InGraphNew, const FRevisionInfo& InRevisionOld, const FRevisionInfo& InRevisionNew)
	...
	, RevisionNew(InRevisionNew)
{
	...
	if (InGraphNew)
	{
		OnGraphChangedDelegateHandle = InGraphNew->AddOnGraphChangedHandler(FOnGraphChanged::FDelegate::CreateRaw(this, &FNarrativeGraphToDiff::OnGraphChanged));
	}

	BuildDiffSourceArray();
}

FNarrativeGraphToDiff::~FNarrativeGraphToDiff()
{
	if (GraphNew)
	{
...
void FNarrativeGraphToDiff::GenerateTreeEntries(TArray<TSharedPtr<FBlueprintDifferenceTreeEntry>>& OutTreeEntries, TArray<TSharedPtr<FBlueprintDifferenceTreeEntry>>& OutRealDifferences)
{
	...
	for (const TSharedPtr<FDiffResultItem>& Difference : DiffListSource)
	{
		TSharedPtr<FBlueprintDifferenceTreeEntry> ChildEntry = MakeShared<FBlueprintDifferenceTreeEntry>(
			FOnDiffEntryFocused::CreateRaw(DiffWidget, &SNarrativeDiff::OnDiffListSelectionChanged, Difference),
	...
	const TSharedPtr<FBlueprintDifferenceTreeEntry> Entry = MakeShared<FBlueprintDifferenceTreeEntry>(
		FOnDiffEntryFocused::CreateRaw(DiffWidget, &SNarrativeDiff::OnGraphSelectionChanged, TSharedPtr<FNarrativeGraphToDiff>(AsShared()), ESelectInfo::Direct),
		FGenerateDiffEntryWidget::CreateSP(AsShared(), &FNarrativeGraphToDiff::GenerateCategoryWidget),
		Children);
	OutTreeEntries.Push(Entry);
}

FText FNarrativeGraphToDiff::GetToolTip() const
{
...
TSharedRef<SWidget> FNarrativeGraphToDiff::GenerateCategoryWidget() const
{
...
void FNarrativeGraphToDiff::BuildDiffSourceArray()
{
	FoundDiffs->Empty();
...
void FNarrativeGraphToDiff::OnGraphChanged(const FEdGraphEditAction& Action) const
{
