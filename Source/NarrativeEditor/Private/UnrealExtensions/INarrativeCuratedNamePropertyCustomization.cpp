...
TSharedPtr<FText> INarrativeCuratedNamePropertyCustomization::NoneAsText = nullptr;

void INarrativeCuratedNamePropertyCustomization::Initialize()
{
...
void INarrativeCuratedNamePropertyCustomization::CreateHeaderRowWidget(FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	HeaderRow
		...
		[
			...
			.OnGenerateWidget_Static(&INarrativeCuratedNamePropertyCustomization::GenerateTextListWidget)
			.OnComboBoxOpening(this, &INarrativeCuratedNamePropertyCustomization::OnTextListComboBoxOpening)
			.OnSelectionChanged(this, &INarrativeCuratedNamePropertyCustomization::OnTextSelected)
			[
				SNew(STextBlock)
				.Text(this, &INarrativeCuratedNamePropertyCustomization::GetCachedText)
				.Font(IDetailLayoutBuilder::GetDetailFont())
				.ToolTipText(this, &INarrativeCuratedNamePropertyCustomization::GetCachedText)
			]
		];
	...
	// Hook-up the ResetToDefault overrides
	{
		const FIsResetToDefaultVisible IsResetVisible =
			FIsResetToDefaultVisible::CreateSP(
				this,
				&INarrativeCuratedNamePropertyCustomization::CustomIsResetToDefaultVisible);

		const FResetToDefaultHandler ResetHandler =
			FResetToDefaultHandler::CreateSP(
				this,
				&INarrativeCuratedNamePropertyCustomization::CustomResetToDefault);
...
bool INarrativeCuratedNamePropertyCustomization::CustomIsResetToDefaultVisible(TSharedPtr<IPropertyHandle> Property) const
{
	const FName CuratedName = GetCuratedName();
	return !CuratedName.IsNone();
}

void INarrativeCuratedNamePropertyCustomization::CustomResetToDefault(TSharedPtr<IPropertyHandle> Property)
{
...
bool INarrativeCuratedNamePropertyCustomization::TrySetCuratedNameWithSideEffects(const FName& NewName)
{
...
FText INarrativeCuratedNamePropertyCustomization::GetCachedText() const
{
	check(CachedTextSelected.IsValid());
...
TSharedRef<SWidget> INarrativeCuratedNamePropertyCustomization::GenerateTextListWidget(const TSharedPtr<FText> InItem)
{
	return
		SNew(STextBlock)
		.Text(*InItem)
...
void INarrativeCuratedNamePropertyCustomization::OnTextListComboBoxOpening()
{
	check(CachedTextSelected.IsValid());
...
void INarrativeCuratedNamePropertyCustomization::OnTextSelected(const TSharedPtr<FText> NewSelection, ESelectInfo::Type SelectInfo)
{
...
TSharedPtr<FText> INarrativeCuratedNamePropertyCustomization::FindCachedOrCreateText(const FName& NewName)
{
	if (NewName.IsNone())
	{
		return NoneAsText;
...
void INarrativeCuratedNamePropertyCustomization::InsertAtHeadOfCachedTextList(const TSharedPtr<FText> Text)
{
...
void INarrativeCuratedNamePropertyCustomization::AddToCachedTextList(const TSharedPtr<FText> Text)
{
	CachedTextList.AddUnique(Text);
}

void INarrativeCuratedNamePropertyCustomization::RepaintTextListWidget() const
{
